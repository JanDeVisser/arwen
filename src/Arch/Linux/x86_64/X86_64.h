...
enum class X86_64ErrorCode {
    InternalError,
    IOError,
};

struct X86_64Error {
    X86_64ErrorCode code;
    std::string     message;
...
struct Object {
    ...
    std::expected<bool, X86_64Error> save_and_assemble() const;
...
struct Executable {
    ...
    std::expected<void, X86_64Error> generate();
};
...
std::expected<void, X86_64Error> generate_x86_64(IR::IRNodes const &ir);
